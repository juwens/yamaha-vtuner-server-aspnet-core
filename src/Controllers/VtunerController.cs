...
namespace VtnrNetRadioServer.Controllers
{
    public class VtunerController : Controller
    {
        ...
        public async Task<ActionResult> FavXML_asp()
        {
            return View("ListOfItemsXml", await _stationsRepo.GetAllAsync());
        }

        [HttpGet("/setupapp/Yamaha/asp/BrowseXML/loginXML.asp")]
        public async Task<ActionResult> loginXML_asp(int? token, string mac, string fver, string dlang, int? startitems, int? enditems)
        {
            if (token == 0) {
